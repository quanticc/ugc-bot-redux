...
//        // pad min and max by 2%, checking if the range is zero
    ...
    public static class DefaultFormatter extends StringConverter<Long> {
        ...
        private DefaultFormatter() {
            formatter = new TimeStringConverter("dd-MM-yy");
        }
        ...
        private static TimeStringConverter getFormatter(int rangeIndex) {
            if (rangeIndex < 0) {
                return new TimeStringConverter(Locale.ENGLISH, "dd-MM-yy");
            } else if (rangeIndex >= TICK_UNIT_FORMATTER_DEFAULTS.length) {
                return new TimeStringConverter(Locale.ENGLISH, TICK_UNIT_FORMATTER_DEFAULTS[TICK_UNIT_FORMATTER_DEFAULTS.length - 1]);
            } else {
                return new TimeStringConverter(Locale.ENGLISH, TICK_UNIT_FORMATTER_DEFAULTS[rangeIndex]);
            }
        }
